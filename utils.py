...
def get_predict(model, model_for_people, img, img_for_people, height, width):
  ...
  class_indices = people_prediction["instances"].pred_classes.cpu().numpy() # Берем индексы классов для элементов предикта
  mask_result = people_prediction["instances"].pred_masks.cpu().numpy() # Берем карты сегментации для каждого предикта
  ...
  for y in range(1, pred.shape[0]-1):
    for x in range(1, pred.shape[1]-1):
      ...
      if point_surrounding[pred[y,x]] < 2 and pred[y,x] not in [3, 4, 5, 10]:
        pred[y,x] = [k for k, v in sorted(point_surrounding.items(), key=lambda item: item[1], reverse=True)][0]
